...
function Days() {
  ...
  // emit "today"
  this.el.find('.today').click(function () {
    self.emit('today');
    return false;
  });
}
...
Days.setMonths = function (datas) {
  months = datas;
};

Days.setDays = function (datas) {
  days = datas;
};
...
Days.prototype.show = function(date){
  ...
  this.head.append(this.renderHeading(3));
  this.body.empty();
  this.body.append(this.renderDays(date));
};

Days.prototype.showEvents = function (events, formater) {
  var map = {};
  for (var k in events) {
    var dates = k.split('-');
    var date = dates[0] + '-' + (parseInt(dates[1], 10) - 1) + '-' + parseInt(dates[2], 10);
    map[date] = events[k];
  }
  this.body.find('td').each(function () {
    var td = o(this);
    var date = td.find('a').data('date');
    var evs = map[date];
    ...
    for (var i = 0; i < evs.length; i++) {
      rows.push('<li>' + (formater ? formater(evs[i]) : evs[i]) + '</li>');
    }
    rows.push('</ul>');
    td.append(rows.join(''));
  });
...
Days.prototype.range = function () {
  var first = this.body.find('td:first a').data('date').split('-');
  first = new Date(first[0], first[1], first[2]);
  var last = this.body.find('td:last a').data('date').split('-');
  last = new Date(last[0], last[1], last[2]);
  return {
    first: first,
    last: last
  };
...
Days.prototype.rowsFor = function(date){
  ...
  return inGroupsOf(cells, perRow);
};
